...
**Status:** `cli` partial · `tui` M7 · `server` M6 · `mcp` M10
...
**Status:** package exists, no tools implemented yet
...
**Status:** package exists, unimplemented
...
**May not import** — `docmax.tools`, `docmax.cli`, `docmax.cloud_client`, any
interface, `rich`, `textual`, `typer`, and — once those layers exist —
`fastapi` and `mcp`.
