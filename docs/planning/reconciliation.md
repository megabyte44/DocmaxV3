...
> **Superseded by events.** ADR 0007 planned to absorb `m1-foundations` component
> by component. It had in fact already been merged into `main` via PR #1 and
> released, which [ADR 0009](../adr/0009-main-is-the-base.md) records. The
> component table below is therefore **already resolved** — everything in it is
> on this branch — and the analysis is kept only because its *evidence* still
> explains why `protocols.py` is the one file that came from the other line.
>
> **Delete this file** once nobody needs that explanation.
...
| `docs/architecture.md` edits | superseded | — | **discard** | `architecture.md` has since moved to `architecture/overview.md` and been rewritten |
