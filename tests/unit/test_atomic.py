...
what happens when something goes *wrong* — and nothing about the happy path
demonstrates it.
...
def target(destination: Path) -> OutputTarget:
    ...
    ``OutputTarget.resolve`` is tested separately; these tests are about what
    happens after a destination has already been approved.
    """
    return OutputTarget(destination=destination, force=True)


def staged_files(directory: Path) -> list[str]:
    """Anything the writers left behind."""
...
def test_atomic_write_delivers_the_bytes(workspace: Path) -> None:
...
def test_a_crash_mid_write_leaves_the_previous_file(
    ...
    from ``Exception``, so a handler written as ``except Exception`` would pass
    the first case and leak a temp file on the second — which is the more common
    one in a terminal tool.
    """
    destination = workspace / "out.pdf"
...
def test_a_failing_validator_prevents_delivery(workspace: Path) -> None:
...
def test_a_validator_inspects_the_staged_file_not_the_destination(workspace: Path) -> None:
    destination = workspace / "out.pdf"
...
def test_an_unexpected_validator_error_becomes_a_typed_one(workspace: Path) -> None:
...
def test_atomic_path_delivers_external_output(workspace: Path) -> None:
    destination = workspace / "compressed.pdf"
...
def test_atomic_path_tolerates_a_program_that_recreates_the_file(workspace: Path) -> None:
...
def test_atomic_path_refuses_to_deliver_nothing(
    workspace: Path, scenario: str, action: Callable[[Path], None]
) -> None:
...
def test_atomic_dir_delivers_a_whole_tree(workspace: Path) -> None:
    destination = workspace / "pages"
...
def test_atomic_dir_replaces_an_existing_tree_completely(workspace: Path) -> None:
...
def test_cancelling_a_multi_file_run_keeps_the_old_tree(workspace: Path) -> None:
...
def test_a_cancelled_nested_tree_is_cleaned_up(workspace: Path) -> None:
