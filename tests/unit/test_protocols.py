...
def test_null_progress_accepts_every_call_and_returns_nothing() -> None:
    """It exists to delete a branch, so it must never be the thing that raises."""
    sink = NullProgress()

    assert sink.start("work", total=10) is None
    assert sink.advance(5) is None
    assert sink.finish() is None
    assert sink.start("indeterminate") is None
...
def test_a_strategy_observes_the_cancellation_it_is_given(tmp_path: Path) -> None:
    ...
    try:
        ...
        raise AssertionError("a cancelled token must stop the operation")
