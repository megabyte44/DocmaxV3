...
— so "fires its callbacks once" has to hold under a race, not merely in
sequence.
...
def test_raise_if_cancelled_names_the_operation() -> None:
    """The message is the one a user sees, so it says what stopped."""
    token = CancellationToken()
    token.cancel()
...
def test_cancellation_is_not_the_users_fault() -> None:
    """It is a completed instruction, not a failure — so no "report this" prompt."""
    token = CancellationToken()
    token.cancel()
...
def test_the_shared_token_does_not_accumulate_callbacks() -> None:
    """It outlives every caller, so a stored callback is a leak for the process."""
    for _ in range(1000):
