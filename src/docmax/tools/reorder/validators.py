"""Checks run against the staged output of ``reorder``, before it replaces anything.
...
def is_readable_pdf(produced: Path) -> None:
    ...
    try:
        ...
        raise OutputValidationError(
            f"The output of 'reorder' could not be reopened as a PDF: {exc}",
            context={"path": str(produced)},
        ) from exc

    if pages == 0:
        raise OutputValidationError(
            "The output of 'reorder' has no pages.",
            context={"path": str(produced)},
        )
