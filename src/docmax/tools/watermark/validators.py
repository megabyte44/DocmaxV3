"""Checks run against the staged output of ``watermark``, before it replaces anything.
...
def is_readable_pdf(produced: Path) -> None:
    ...
    try:
        ...
        raise OutputValidationError(
            f"The output of 'watermark' could not be reopened as a PDF: {exc}",
            context={"path": str(produced)},
        ) from exc

    if pages == 0:
        raise OutputValidationError(
            "The output of 'watermark' has no pages.",
            context={"path": str(produced)},
        )
...
def page_count_is(expected: int) -> Validator:
    ...
    Marking pages must not change how many there are. An overlay merge that
    took a page with it would be the failure a user is least likely to notice,
    because every page that remains looks exactly right.
    """
