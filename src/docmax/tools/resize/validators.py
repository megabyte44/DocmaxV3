"""Checks run against the staged output of ``resize``, before it replaces anything.
...
def is_readable_image(expected_format: str) -> Validator:
    ...
    on the output format, which is determined by the destination suffix.
    """
    ...
    def validate(produced: Path) -> None:
        ...
        try:
            ...
            raise OutputValidationError(
                f"The output of 'resize' could not be reopened as an image: {exc}",
