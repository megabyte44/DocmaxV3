"""Checks run against the staged output of ``metadata``, before it replaces anything.
...
def is_readable_pdf(produced: Path) -> None:
    ...
    try:
        ...
        raise OutputValidationError(
            f"The output of 'metadata' could not be reopened as a PDF: {exc}",
            context={"path": str(produced)},
        ) from exc

    if pages == 0:
        raise OutputValidationError(
            "The output of 'metadata' has no pages.",
            context={"path": str(produced)},
        )
